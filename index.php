<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <!-- <title>Khalifa University – IEEE EMBS</title> -->
  <title>
    8th International Conference on Environmental Applications of Advanced
    Oxidation Processes (EAAOP-8)
  </title>

  <!-- Favicon for standard browsers -->
  <link rel="icon" href="assets/images/favicon.png" type="image/x-icon" />

  <!-- Owl Carousel CSS -->
  <link
    rel="stylesheet"
    href="https://cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.3.4/assets/owl.carousel.min.css" />
  <link
    rel="stylesheet"
    href="https://cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.3.4/assets/owl.theme.default.min.css" />

  <!-- Our custom SCSS (compiled to CSS) -->
  <link rel="stylesheet" href="assets/css/main.css" />
</head>

<body>
  <!--  header SECTION  -->
  <?php include 'includes/header.php'; ?>

  <!-- <header id="navbar" class="ku-header">
      <div class="container">
        <div class="ku-header__content">
          <div class="ku-header__left">
            <div>
              <a href="index.html">
                <img
                  src="assets/images/ku-logo.png"
                  alt="Khalifa University Logo"
                  class="ku-header__logo"
                />
              </a>
            </div>
          </div>

          <div class="ku-header__navs">
            <img
              src="assets/images/logo2.png"
              alt="Khalifa University Logo"
              class="ku-header__logo"
            />
            <nav class="ku-header__nav">
              <div>
                <ul class="ku-nav__list">
                  <li><a href="#home" class="ku-nav__link">HOME</a></li>
                  <li>
                    <a href="committees.html" class="ku-nav__link"
                      >COMMITTEEs</a
                    >
                  </li>
                  <li>
                    <a href="speakers.php" class="ku-nav__link">SPEAKERS</a>
                  </li>
                  <li><a href="#program" class="ku-nav__link">PROGRAM</a></li>
                  <li>
                    <a href="#registration" class="ku-nav__link"
                      >registration</a
                    >
                  </li>
                </ul>
                <ul class="ku-nav__list" style="margin-top: 0.3rem">
                  <li><a href="#call-for-abstract">submit Abstract</a></li>
                  <li><a href="#special-issues">special issues</a></li>

                  <li><a href="#venue" class="ku-nav__link">venue</a></li>
                  <li>
                    <a href="#sponsorship" class="ku-nav__link">sponsors</a>
                  </li>
                </ul>
              </div>

              <button>
                <a href="mailto:eaaop8@ku.ac.ae" class="button-shape-contact"
                  >CONTACT US</a
                >
              </button>
            </nav>

            <button
              class="ku-header__menu-toggle"
              id="menuToggle"
              aria-label="Menu"
            >
              <span></span><span></span><span></span>
            </button>
          </div>
        </div>

        <nav class="ku-header__nav2">
          <div>
            <ul class="ku-nav__list">
              <li><a href="#home" class="ku-nav__link">HOME</a></li>
              <li>
                <a href="committees.html" class="ku-nav__link">COMMITTEEs</a>
              </li>
              <li>
                <a href="speakers.html" class="ku-nav__link">SPEAKERS</a>
              </li>
              <li><a href="#program" class="ku-nav__link">PROGRAM</a></li>
              <li>
                <a href="#registration" class="ku-nav__link">registration</a>
              </li>
              <li><a href="#call-for-abstract">submit Abstract</a></li>
              <li><a href="#special-issues">special issues</a></li>

              <li><a href="#venue" class="ku-nav__link">venue</a></li>
              <li>
                <a href="#sponsorship" class="ku-nav__link">sponsors</a>
              </li>
            </ul>
          </div>

          <button>
            <a href="mailto:eaaop8@ku.ac.ae" class="button-shape-contact"
              >CONTACT US</a
            >
          </button>
        </nav>
      </div>
    </header> -->

  <!-- Menu -->
  <div
    class="menu-model"
    id="mobileMenu"
    role="dialog"
    aria-modal="true"
    aria-label="Mobile Navigation">
    <aside class="mobile-menu" aria-label="Mobile menu">
      <div>
        <button class="close-btn" id="closeMenu" aria-label="Close menu">
          <img
            src="assets/images/close.png"
            alt="Close Menu Icon"
            class="close-icon" />
        </button>
      </div>

      <div class="menu-content">
        <ul class="nav-menu">
          <li><a href="#home">HOME</a></li>
          <li><a href="#about">ABOUT</a></li>
          <li>
            <a href="committees.php">COMMITTEES</a>
          </li>
          <li>
            <a href="speakers.php">SPEAKERS</a>
          </li>
          <li><a href="#program">PROGRAM</a></li>
          <li>
            <a href="#registration">registration</a>
          </li>
          <li><a href="#call-for-abstract">submit Abstract</a></li>
          <li><a href="#special-issues">special issues</a></li>

          <li><a href="#venue">venue</a></li>
          <li>
            <a href="#sponsorship">sponsors</a>
          </li>
        </ul>
      </div>
      <div>
        <a
          href="mailto:eaaop8@ku.ac.ae"
          class="button-shape contact-btn button-shape-primary">CONTACT US</a>
      </div>
    </aside>
  </div>

  <!-- hero section  -->
  <section id="home" class="hero-section">
    <div class="container hero-section__content">
      <div
        class="hero-section__left animate-on-scroll"
        data-animate="fade-right">
        <div class="hero-section__logo-section">
          <img
            id="hero_logo"
            src="assets/images/EAAOP8-LOGO.png"
            alt="Khalifa University Logo"
            class="hero-section__logo" />
        </div>
        <h1 class="hero-section__title">
          8<sup class="power">th</sup> International Conference on
          Environmental Applications of Advanced Oxidation Processes (EAAOP-8)
        </h1>
        <h2 class="hero-section__subtitle">
          Abu Dhabi, United Arab Emirates, 31st March 2027 – 2nd April 2027
        </h2>
        <div>
          <button>
            <a href="#registration" class="button-shape button-shape-primary">
              registration
            </a>
          </button>
        </div>
      </div>
      <div
        class="hero-section__right animate-on-scroll"
        data-animate="fade-left">
        <div class="bg-image">
          <img
            src="assets/images/hero1.png"
            alt="hero"
            class="bg-image__hero-img bg-image__hero-img1" />
        </div>
        <div class="bg-image bg-image2">
          <img
            src="assets/images/hero2.png"
            alt="hero"
            class="bg-image__hero-img bg-image2__hero-img2" />
        </div>
        <div class="bg-image bg-image3">
          <img
            src="assets/images/hero3.png"
            alt="hero"
            class="bg-image__hero-img bg-image__hero-img3" />
          <div class="bg-color"></div>
        </div>
      </div>
    </div>
  </section>

  <!-- about section  -->
  <section id="about" class="about">
    <div class="container about__content">
      <div class="about__info">
        <p
          class="about__description animate-on-scroll"
          data-animate="fade-up">
          On behalf of the Organizing Committee, it is our great pleasure to
          welcome you to the 8th International Conference on Environmental
          Applications of Advanced Oxidation Processes (EAAOP-8), which will
          take place in Abu Dhabi, United Arab Emirates, 31st March 2027 – 2nd
          April 2027.
        </p>
        <div class="about__image animate-on-scroll" data-animate="fade-right">
          <img src="assets/images/about.png" alt="about" />
        </div>
      </div>

      <div class="about__info">
        <p
          class="about__description animate-on-scroll"
          data-animate="fade-up">
          The conference is organised by Khalifa University and stands as a
          relevant event and network opportunity for sharing knowledge and
          expertise among scientists, academia, environmental professionals
          and industry, policymakers and other relevant stakeholders, in
          recent high impact investigations and technological developments on
          environmental applications of advanced oxidation processes (AOPs)
          and related areas. <br />
          <br />

          Scientific and technological progress of AOPs in air, water,
          wastewater and soil treatment, as well as valorisation and recovery
          of aqueous matrices, biomass and wastes, are welcome.
          <br />
          <br />
          In a constantly evolving regulatory context, AOPs are expected to
          play an increasingly important role in successfully addressing new
          environmental challenges. Therefore, we are sure that the EAAOP-8
          conference will be an important opportunity to demonstrate how AOPs
          can significantly contribute to this aim, as well as of discussion
          among all the stakeholders.
        </p>

        <p
          class="about__description about__bold animate-on-scroll"
          data-animate="fade-up">
          The conference will be followed by the International PhD School of
          Advanced Oxidation Processes taking place on
          <strong>5th-6th April 2027</strong>
        </p>

        <div
          class="speaker-content animate-on-scroll"
          data-animate="fade-left">
          <p class="speaker-content__message">
            Looking forward to meeting you in Abu Dhabi
          </p>
          <div class="speaker-cards">
            <figure class="speaker-card" aria-label="Event Speaker">
              <div class="speaker-card__image">
                <img
                  src="assets/images/member-1.png"
                  alt="Portrait of Giovanni Palmisano"
                  loading="lazy" />
              </div>
              <figcaption class="speaker-card__content">
                <h3 class="speaker-card__name">
                  Giovanni Palmisano <br />
                  <span class="speaker-card__role"> Chair </span>
                </h3>
              </figcaption>
            </figure>
            <figure class="speaker-card" aria-label="Event Speaker">
              <div class="speaker-card__image">
                <img
                  src="assets/images/member-2.png"
                  alt="Portrait of Lianxi Zheng"
                  loading="lazy" />
              </div>
              <figcaption class="speaker-card__content">
                <h3 class="speaker-card__name">
                  Lianxi Zheng <br /><span class="speaker-card__role">
                    Co-Chair
                  </span>
                </h3>
              </figcaption>
            </figure>
          </div>
        </div>

        <div>
          <a
            href="#registration"
            target="_blank"
            class="button-shape button-shape-black animate-on-scroll"
            data-animate="fade-up">
            registration
          </a>
        </div>
      </div>
    </div>
  </section>

  <!-- conference section  -->
  <section class="conference-section">
    <div class="container">
      <h1
        class="conference-section__title animate-on-scroll"
        data-animate="fade-right">
        Conference Themes
      </h1>
      <div
        class="conference-section__grid animate-on-scroll"
        data-animate="fade-up">
        <ul class="conference-section__list">
          <li>Novel catalytic materials for environmental applications</li>
          <li>In situ and operando measurements</li>
          <li>
            Advanced oxidation processes (water and air treatment, soil
            remediation, production of energy, and value-added chemicals)
          </li>
          <li>
            Photocatalysis for environmental and energy applications, and
            organic synthesis
          </li>
          <li>Disinfection</li>
          <li>Fenton-based processes</li>
          <li>Ozonation</li>
        </ul>
        <ul class="conference-section__list">
          <li>
            Electrochemical processes for environmental, energy applications,
            and organic synthesis
          </li>
          <li>Wet air and supercritical oxidation</li>
          <li>Catalytic redox processes</li>
          <li>Plasma technologies</li>
          <li>Pilot-scale studies and field applications</li>
          <li>Process coupling/intensification</li>
        </ul>
      </div>
    </div>
  </section>

  <!-- who-should-attend -->
  <section class="who-should-attend">
    <div class="container">
      <h1
        class="who-should-attend__title animate-on-scroll"
        data-animate="fade-up">
        Who Should Attend
      </h1>
      <div class="who-should-attend__cards">
        <div class="card animate-on-scroll" data-animate="fade-up">
          <div class="card__icon">
            <img src="assets/images/academic.png" alt="Academic Scholars" />
          </div>
          <p class="card__text">
            Academic scholars and graduate students from universities and
            research institutions with interests in environmental and energy
            technologies, green chemistry, and sustainable remediation.
          </p>
        </div>
        <div class="card animate-on-scroll" data-animate="fade-up">
          <div class="card__icon">
            <img
              src="assets/images/industry.png"
              alt="Industry Professionals" />
          </div>
          <p class="card__text">
            Industry professionals working in sectors such as water treatment,
            air pollution control, chemical manufacturing, and environmental
            engineering.
          </p>
        </div>
        <div class="card animate-on-scroll" data-animate="fade-up">
          <div class="card__icon">
            <img
              src="assets/images/environment.png"
              alt="Utility Representatives" />
          </div>
          <p class="card__text">
            Environmental utility company representatives, particularly those
            focused on the implementation of innovative oxidation processes
            for real-world applications.
          </p>
        </div>
        <div class="card animate-on-scroll" data-animate="fade-up">
          <div class="card__icon">
            <img
              src="assets/images/consultant.png"
              alt="Consultants and Practitioners" />
          </div>
          <p class="card__text">
            Independent consultants and practitioners involved in
            environmental project development, technology assessment, and
            policy advising.
          </p>
        </div>
      </div>
    </div>
  </section>

  <!-- important-dates -->
  <section class="important-dates">
    <div class="container important-dates__content">
      <div
        class="important-dates__grid__left animate-on-scroll"
        data-animate="fade-right">
        <img
          src="assets/images/important.png"
          alt="important-dates"
          class="important-dates__grid__left__img" />
      </div>
      <div
        class="important-dates__grid__right animate-on-scroll"
        data-animate="fade-left">
        <h1 class="important-dates__title">Important Dates</h1>
        <ul class="important-dates__list">
          <li class="important-dates__item">
            <div class="date">
              <span class="date__from">From</span>
              <span class="date__day">01</span>
              <span class="date__month">Oct</span>
            </div>
            <div class="event">
              <span class="event__year">2025</span>
              <span class="event__title">Abstract Submission Opens</span>
            </div>
          </li>

          <li class="important-dates__item">
            <div class="date">
              <span class="date__from">From</span>
              <span class="date__day">01</span>
              <span class="date__month">Jun</span>
            </div>
            <div class="event">
              <span class="event__year">2026</span>
              <span class="event__title">Communication of Abstract Acceptance (Oral/Flash
                Oral/Poster)</span>
            </div>
          </li>

          <li class="important-dates__item">
            <div class="date">
              <span class="date__day">15</span>
              <span class="date__month">Dec</span>
            </div>
            <div class="event">
              <span class="event__year">2026</span>
              <span class="event__title">Abstract Submission Deadline</span>
            </div>
          </li>

          <li class="important-dates__item">
            <div class="date">
              <span class="date__day">01</span>
              <span class="date__month">Nov</span>
            </div>
            <div class="event">
              <span class="event__year">2026</span>
              <span class="event__title">Early Bird Registration Deadline</span>
            </div>
          </li>

          <li class="important-dates__item">
            <div class="date">
              <span class="date__day">15</span>
              <span class="date__month">Feb</span>
            </div>
            <div class="event">
              <span class="event__year">2027</span>
              <span class="event__title">Regular Registration Deadline</span>
            </div>
          </li>
        </ul>
      </div>
    </div>
  </section>

  <!-- Program Section -->
  <section id="program" class="program">
    <div class="container">
      <div class="program__content">
        <!-- Left: Header -->
        <div
          class="program__header animate-on-scroll"
          data-animate="fade-right">
          <h1 class="program__title">Preliminary Program</h1>
        </div>

        <!-- Right: Banner -->
        <div
          class="program__banner animate-on-scroll"
          data-animate="fade-left">
          <img
            src="assets/images/program-banner.png"
            alt="Scientific Program" />
        </div>
      </div>

      <!-- Presentation Types -->
      <div class="program__presentations">
        <h3
          class="program__presentations__title animate-on-scroll"
          data-animate="fade-right">
          Types of presentation
        </h3>
        <div class="program__grid animate-on-scroll" data-animate="fade-up">
          <div class="program__item">
            <img
              src="assets/images/plenary-lecture.png"
              alt="Plenary lecture" />
            <div>
              <h4>Plenary lecture</h4>
              <p>40 min (+ 5 min Q/A)</p>
            </div>
          </div>

          <div class="program__item">
            <img
              src="assets/images/keynote-lecture.png"
              alt="Keynote lecture" />
            <div>
              <h4>Keynote lecture</h4>
              <p>20 min (+ 5 min Q/A)</p>
            </div>
          </div>

          <div class="program__item">
            <img
              src="assets/images/invited-presentation.png"
              alt="Invited presentation" />
            <div>
              <h4>Invited presentation</h4>
              <p>16 min (+4 min Q/A)</p>
            </div>
          </div>

          <div class="program__item">
            <img
              src="assets/images/oral-presentation.png"
              alt="Oral presentation" />
            <div>
              <h4>Oral presentation</h4>
              <p>12 min (+3 min Q/A)</p>
            </div>
          </div>

          <div class="program__item">
            <img
              src="assets/images/flash-presentation.png"
              alt="Flash presentation" />
            <div>
              <h4>Flash presentation</h4>
              <p>5 min</p>
            </div>
          </div>

          <div class="program__item">
            <img src="assets/images/poster.png" alt="Poster presentation" />
            <div>
              <h4>Poster size</h4>
              <p>A0</p>
            </div>
          </div>
        </div>
      </div>

      <!-- special-issues -->
      <section id="special-issues" class="special-issues">
        <h3
          class="special-issues__title animate-on-scroll"
          data-animate="fade-right">
          Special Issues
        </h3>

        <div class="special-issues__details">
          <p
            class="special-issues__text animate-on-scroll"
            data-animate="fade-right">
            The organizing committee is submitting the request to have
            dedicated special issues in the following journals:
          </p>

          <div
            class="special-issues__list animate-on-scroll"
            data-animate="fade-left">
            <ul class="special-issues__column">
              <li>Chemical Engineering Journal</li>
              <li>Catalysis Today</li>
              <li>Journal of Environmental Management</li>
            </ul>
            <ul class="special-issues__column">
              <li>Case Studies in Chemical and Environmental Engineering</li>
              <li>Journal of Environmental Chemical Engineering</li>
            </ul>
          </div>
        </div>
      </section>

      <!-- tutorial-sessions -->
      <section class="tutorial-sessions">
        <div class="tutorial-sessions__details">
          <img
            src="assets/images/insititute-1.png"
            alt="insititute "
            loading="lazy"
            class="tutorial-sessions__image animate-on-scroll"
            data-animate="fade-right" />

          <div
            class="tutorial-sessions__content animate-on-scroll"
            data-animate="fade-left">
            <h3 class="tutorial-sessions__title">
              Tutorial and Special Sessions
            </h3>
            <ul class="tutorial-sessions__list">
              <li>Coffee with Editors</li>
              <li>
                International AOP School
                <span>(taking place on 5th–6th April 2027)</span>
              </li>
            </ul>
          </div>
        </div>
      </section>

      <!-- Schedule Component -->
      <div class="schedule">
        <!-- Date Tabs -->
        <div style="text-align: center">
          <div
            class="schedule__tabs animate-on-scroll"
            data-animate="fade-up"
            role="tablist">
            <button
              role="tab"
              aria-selected="true"
              class="schedule__tab schedule__tab--active"
              data-date="2027-03-31">
              31st March 2027
            </button>
            <button
              role="tab"
              aria-selected="false"
              class="schedule__tab"
              data-date="2027-04-01">
              1st April 2027
            </button>
            <button
              role="tab"
              aria-selected="false"
              class="schedule__tab"
              data-date="2027-04-02">
              2nd April 2027
            </button>
          </div>
        </div>

        <!-- Schedule Body -->
        <div class="schedule__body">
          <!-- Room Selector -->
          <div
            class="schedule__rooms animate-on-scroll"
            data-animate="fade-right">
            <button
              class="schedule__room-btn schedule__room-btn--active"
              data-room="auditorium">
              Auditorium <br />
              <span>C00068-C01066</span>
            </button>
            <button class="schedule__room-btn" data-room="b00055">
              Lecture room <br />
              <span>B00055</span>
            </button>
            <button class="schedule__room-btn" data-room="b00107">
              Lecture room <br />
              <span>B00107</span>
            </button>
          </div>

          <!-- Room Schedules -->
          <div
            class="schedule__content animate-on-scroll"
            data-animate="fade-up">
            <!--  Content for 31st March -->
            <section>
              <!-- Auditorium -->
              <div
                class="schedule__room-content schedule__room-content--active"
                data-date="2027-03-31"
                data-room="auditorium">
                <ul class="schedule__list">
                  <li class="schedule__slot">
                    08:30 – 09:00 | Opening Ceremony
                  </li>
                  <li class="schedule__slot">09:00 – 09:45 | PL1</li>
                  <li class="schedule__slot">09:45 – 10:30 | KN1, IS1</li>
                  <li class="schedule__slot">
                    10:30 – 10:50 | Coffee Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                  <li class="schedule__slot">10:50 – 12:30 | OP1-6, FP1-2</li>
                  <li class="schedule__slot">
                    12:30 – 13:30 | Lunch Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                  <li class="schedule__slot">13:30 – 15:00 | OP7-12</li>
                  <li class="schedule__slot">
                    15:00 – 16:00 | Coffee Break (Multi-Purpose Hall D00073)
                    and Poster Presentations (Spine Area)
                  </li>
                  <li class="schedule__slot">
                    16:00 – 18:00 | OP13-19, FP3-5
                  </li>
                  <li class="schedule__slot">
                    18:00 – 19:00 | Welcome reception (Multi-Purpose Hall
                    D00073)
                  </li>
                </ul>
              </div>

              <!-- b00055 -->
              <div
                class="schedule__room-content"
                data-date="2027-03-31"
                data-room="b00055">
                <ul class="schedule__list">
                  <li class="schedule__slot">09:45 – 10:30 | KN2, IS2</li>
                  <li class="schedule__slot">
                    10:30 – 10:50 | Coffee Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                  <li class="schedule__slot">
                    10:50 – 12:30 | OP20-25, FP6-7
                  </li>
                  <li class="schedule__slot">
                    12:30 – 13:30 | Lunch Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                  <li class="schedule__slot">13:30 – 15:00 | OP26-31</li>
                  <li class="schedule__slot">
                    15:00 – 16:00 | Coffee Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                    and Poster Presentations
                    <span class="schedule__location">(Spine Area)</span>
                  </li>
                  <li class="schedule__slot">
                    16:00 – 18:00 | OP32-38, FP8-10
                  </li>
                  <li class="schedule__slot">
                    18:00 – 19:00 | Welcome Reception
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                </ul>
              </div>

              <!-- b00107 -->
              <div
                class="schedule__room-content"
                data-date="2027-03-31"
                data-room="b00107">
                <ul class="schedule__list">
                  <li class="schedule__slot">09:45 – 10:30 | IP1, OP39-40</li>
                  <li class="schedule__slot">
                    10:30 – 10:50 | Coffee Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                  <li class="schedule__slot">
                    10:50 – 12:30 | OP41-46, FP11-12
                  </li>
                  <li class="schedule__slot">
                    12:30 – 13:30 | Lunch Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                  <li class="schedule__slot">13:30 – 15:00 | OP47-52</li>
                  <li class="schedule__slot">
                    15:00 – 16:00 | Coffee Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                    and Poster Presentations
                    <span class="schedule__location">(Spine Area)</span>
                  </li>
                  <li class="schedule__slot">
                    16:00 – 18:00 | OP53-59, FP13-15
                  </li>
                  <li class="schedule__slot">
                    18:00 – 19:00 | Welcome reception
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                </ul>
              </div>
            </section>

            <!-- 1st April -->

            <section>
              <!-- Auditorium -->
              <div
                class="schedule__room-content"
                data-date="2027-04-01"
                data-room="auditorium">
                <ul class="schedule__list">
                  <li class="schedule__slot">09:00 – 09:45 | PL2</li>
                  <li class="schedule__slot">09:45 – 10:30 | KN3, IS3</li>
                  <li class="schedule__slot">
                    10:30 – 10:50 | Coffee Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                  <li class="schedule__slot">
                    10:50 – 12:30 | OP60-65, FP16-17
                  </li>
                  <li class="schedule__slot">
                    12:30 – 13:30 | Lunch Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                  <li class="schedule__slot">13:30 – 15:00 | OP66-71</li>
                  <li class="schedule__slot">
                    15:00 – 15:20 | Coffee Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                  <li class="schedule__slot">
                    15:20 – 17:00 | Round table – Meet the Editors
                  </li>
                  <li class="schedule__slot">
                    17:00 – 19:30 | Abu Dhabi bus tour
                  </li>
                  <li class="schedule__slot">
                    19:30 – 22:00 | Conference Dinner
                  </li>
                </ul>
              </div>

              <!--  b00055 -->
              <div
                class="schedule__room-content"
                data-date="2027-04-01"
                data-room="b00055">
                <ul class="schedule__list">
                  <li class="schedule__slot">09:45 – 10:30 | KN4, IS4</li>
                  <li class="schedule__slot">
                    10:30 – 10:50 | Coffee Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                  <li class="schedule__slot">
                    10:50 – 12:30 | OP72-77, FP18-19
                  </li>
                  <li class="schedule__slot">
                    12:30 – 13:30 | Lunch Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                  <li class="schedule__slot">13:30 – 15:00 | OP78-83</li>
                  <li class="schedule__slot">
                    15:00 – 15:20 | Coffee Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                  <li class="schedule__slot">
                    17:00 – 19:30 | Abu Dhabi bus tour
                  </li>
                  <li class="schedule__slot">
                    19:30 – 22:00 | Conference Dinner
                  </li>
                </ul>
              </div>
              <!-- b00107 -->
              <div
                class="schedule__room-content"
                data-date="2027-04-01"
                data-room="b00107">
                <ul class="schedule__list">
                  <li class="schedule__slot">09:45 – 10:30 | IP2, OP84-85</li>
                  <li class="schedule__slot">
                    10:30 – 10:50 | Coffee Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                  <li class="schedule__slot">
                    10:50 – 12:30 | OP86-91, FP20-21
                  </li>
                  <li class="schedule__slot">
                    12:30 – 13:30 | Lunch Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                  <li class="schedule__slot">13:30 – 15:00 | OP92-97</li>
                  <li class="schedule__slot">
                    15:00 – 15:20 | Coffee Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                  <li class="schedule__slot">
                    17:00 – 19:30 | Abu Dhabi bus tour
                  </li>
                  <li class="schedule__slot">
                    19:30 – 22:00 | Conference Dinner
                  </li>
                </ul>
              </div>
            </section>

            <!-- 2nd April -->
            <section>
              <!-- Auditorium -->
              <div
                class="schedule__room-content"
                data-date="2027-04-02"
                data-room="auditorium">
                <ul class="schedule__list">
                  <li class="schedule__slot">09:00 – 09:45 | PL3</li>
                  <li class="schedule__slot">09:45 – 10:30 | KN5, IS5</li>
                  <li class="schedule__slot">
                    10:30 – 10:50 | Coffee Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                  <li class="schedule__slot">
                    10:50 – 12:30 | OP98-103, FP22-23
                  </li>
                  <li class="schedule__slot">
                    12:30 – 13:30 | Lunch Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                  <li class="schedule__slot">13:30 – 15:00 | OP104-109</li>
                  <li class="schedule__slot">
                    15:00 – 16:00 | Coffee Break (Multi-Purpose Hall D00073)
                    and Poster Presentations (Spine Area)
                  </li>
                  <li class="schedule__slot">
                    16:00 – 18:00 | OP110-116, FP24-26
                  </li>
                  <li class="schedule__slot">
                    18:00 – 18:30 | Final Remarks and Presentation of EAAOP9
                  </li>
                </ul>
              </div>

              <!--  b00055 -->
              <div
                class="schedule__room-content"
                data-date="2027-04-02"
                data-room="b00055">
                <ul class="schedule__list">
                  <li class="schedule__slot">09:45 – 10:30 | KN6, IS6</li>
                  <li class="schedule__slot">
                    10:30 – 10:50 | Coffee Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                  <li class="schedule__slot">
                    10:50 – 12:30 | OP117-122, FP27-28
                  </li>
                  <li class="schedule__slot">
                    12:30 – 13:30 | Lunch Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                  <li class="schedule__slot">13:30 – 15:00 | OP123-128</li>
                  <li class="schedule__slot">
                    15:00 – 16:00 | Coffee Break (Multi-Purpose Hall D00073)
                    and Poster Presentations (Spine Area)
                  </li>
                  <li class="schedule__slot">16:00 – 18:00 | OP129-136</li>
                  <li class="schedule__slot">
                    18:00 – 18:30 | Final Remarks and Presentation of EAAOP9
                  </li>
                </ul>
              </div>
              <!-- b00107 -->
              <div
                class="schedule__room-content"
                data-date="2027-04-02"
                data-room="b00107">
                <ul class="schedule__list">
                  <li class="schedule__slot">
                    09:45 – 10:30 | IP3, OP137-138
                  </li>
                  <li class="schedule__slot">
                    10:30 – 10:50 | Coffee Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                  <li class="schedule__slot">
                    10:50 – 12:30 | OP139-144, FP29-30
                  </li>
                  <li class="schedule__slot">
                    12:30 – 13:30 | Lunch Break
                    <span class="schedule__location">(Multi-Purpose Hall D00073)</span>
                  </li>
                  <li class="schedule__slot">13:30 – 15:00 | OP145-150</li>
                  <li class="schedule__slot">
                    15:00 – 16:00 | Coffee Break (Multi-Purpose Hall D00073)
                    and Poster Presentations (Spine Area)
                  </li>
                  <li class="schedule__slot">16:00 – 18:00 | OP151-158</li>
                  <li class="schedule__slot">
                    18:00 – 18:30 | Final Remarks and Presentation of EAAOP9
                  </li>
                </ul>
              </div>
            </section>
          </div>
        </div>
      </div>
    </div>
  </section>

  <!-- registration section  -->
  <section id="registration" class="registration-section">
    <div class="container">
      <h1 class="registration-title animate-on-scroll" data-animate="fade-up">
        Registration Categories and Fees
      </h1>

      <table
        class="registration-table animate-on-scroll"
        data-animate="fade-up">
        <thead>
          <tr>
            <th>Type</th>
            <th>
              EARLY BIRD<br />
              <span class="date-note">before 1st Nov 2026</span>
            </th>
            <th>
              Regular<br />
              <span class="date-note">after 1st Nov 2026</span>
            </th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td>Regular delegate</td>
            <td>$650</td>
            <td>$750</td>
          </tr>
          <tr>
            <td>Industrial delegate</td>
            <td>$800</td>
            <td>$900</td>
          </tr>
          <tr>
            <td>Student (status certificate required)</td>
            <td>$350</td>
            <td>$450</td>
          </tr>
          <tr>
            <td>Accompanying person</td>
            <td>$250</td>
            <td>$350</td>
          </tr>
        </tbody>
      </table>

      <p class="note animate-on-scroll" data-animate="fade-up">
        Lunches, gala dinner, coffee breaks, welcome reception, social event
        included in the registration fee.
      </p>
      <p class="note animate-on-scroll" data-animate="fade-up">
        Registration to the AOP international school: free for students
        registered at EAAOP8 conference; $150 for students not registered at
        EAAOP8 conference.
      </p>

      <div
        class="registration-buttons animate-on-scroll"
        data-animate="fade-up">
        <a href="#">
          <button class="button-shape button-shape-dark">Register Now</button>
        </a>
      </div>
    </div>
  </section>

  <!-- abstract-call section  -->
  <section id="call-for-abstract" class="abstract-call">
    <div class="container">
      <div class="abstract-call__content">
        <div
          class="abstract-content animate-on-scroll"
          data-animate="fade-right">
          <h1 class="abstract-title">ABSTRACT SUBMISSION</h1>
          <p class="abstract-description">
            The Scientific Program will include poster, oral and flash oral
            communications. Flash oral communications will be complemented by
            a poster.
          </p>
          <br />
          <p class="abstract-description">
            Each participant can submit up to two abstracts as presenting
            author, but only one abstract can be selected for oral
            communication.
          </p>
        </div>

        <div
          class="abstract-image animate-on-scroll"
          data-animate="fade-left">
          <img
            src="assets/images/person-with-laptop.png"
            alt="Person writing near laptop" />
        </div>
      </div>

      <div class="abstract-form">
        <!-- Instructions -->
        <section
          class="abstract-form__note animate-on-scroll"
          data-animate="fade-up">
          <h2 class="abstract-form__title">
            Before Abstract Submission, please take into account the
            following:
          </h2>
          <div class="abstract-form__details">
            <ul>
              <li>
                <img
                  src="assets/images/abstract-language.png"
                  alt="Language icon" />
                <span style="line-height: 1.2"><strong>Abstract language <br /></strong> English</span>
              </li>
              <li>
                <img
                  src="assets/images/abstract-pages.png"
                  alt="Pages icon" />
                <span style="line-height: 1.1"><strong>Abstract must be two pages maximum</strong></span>
              </li>
            </ul>
            <p style="max-width: 85%; font-weight: 700">
              Authors must use the abstract template (<a
                href="assets\docs\EAAOP-8-Abstract-Template.docx"
                target="_blank"
                style="text-decoration: underline">download</a>) and strictly follow the instructions reported in the template
              file. ONLY WORD DOCUMENTS WILL BE ACCEPTED.
            </p>
            <p>
              Name the abstract as
              <span style="font-weight: 600">"EAAOP-8_abstract_author surname"</span>
              or, if the author submits more than one abstract, also number it
              <br />
              <span style="font-weight: 600">"EAAOP-8_abstract_author surname_1"</span>
            </p>
            <p>
              Abstracts not complying with the template <br />
              <span style="font-weight: 600">WILL BE SENT BACK FOR CORRECTION.</span>
            </p>
          </div>
        </section>

        <a href="https://edas.info/newPaper.php?c=34445" target="_blank">
          <button class="btn button-shape button-shape-primary">
            submit Abstract
          </button>
        </a>

        <!-- Form Fields -->
        <!-- <form
            class="abstract-form__fields animate-on-scroll"
            data-animate="fade-up"
          >
            <fieldset class="abstract-form__row">
              <div class="abstract-form__field">
                <input
                  type="text"
                  id="first-name"
                  name="first-name"
                  placeholder="First Name"
                  required
                />
              </div>
              <div class="abstract-form__field">
                <input
                  type="text"
                  id="last-name"
                  name="last-name"
                  placeholder="Last Name"
                  required
                />
              </div>
            </fieldset>

            <fieldset class="abstract-form__row">
              <div class="abstract-form__field">
                <input
                  type="email"
                  id="email"
                  name="email"
                  placeholder="Email"
                  required
                />
              </div>
              <div class="abstract-form__field">
                <input
                  type="text"
                  id="department"
                  name="department"
                  placeholder="Department"
                />
              </div>
            </fieldset>

            <h2 class="abstract-form__section-title">Abstract Information</h2>

            <fieldset class="abstract-form__row">
              <div class="abstract-form__field">
                <input
                  type="text"
                  id="authors"
                  name="authors"
                  placeholder="Author(s)"
                  required
                />
              </div>
              <div class="abstract-form__field">
                <input
                  type="text"
                  id="affiliations"
                  name="affiliations"
                  placeholder="Affiliation(s)"
                  required
                />
              </div>
            </fieldset>

            <fieldset class="abstract-form__row">
              <div class="abstract-form__field">
                <input
                  type="text"
                  id="title"
                  name="title"
                  placeholder="Title of your Paper"
                  required
                />
              </div>
              <div class="abstract-form__field">
                <input
                  type="text"
                  id="length"
                  name="length"
                  placeholder="Abstract Length"
                />
              </div>
            </fieldset>

            <fieldset class="abstract-form__row--full">
              <div class="abstract-form__field abstract-form__field--full">
                <label>Full Abstract</label>
                <textarea
                  id="full-abstract"
                  name="full-abstract"
                  rows="6"
                  required
                ></textarea>
              </div>
            </fieldset>

            <div class="abstract-form__submit">
              <button class="button-shape button-shape-primary">
                Submit Abstract
              </button>
            </div>
          </form> -->
      </div>
    </div>
  </section>

  <!-- sponsor section  -->
  <section id="sponsorship" class="sponsors-section">
    <div class="container">
      <h1 class="sponsors-title animate-on-scroll" data-animate="fade-up">
        Sponsors & Exhibitors
      </h1>

      <div class="sponsor-logos animate-on-scroll" data-animate="fade-up">
        <a href="https://www.elsevier.com/" target="_blank">
          <img
            src="assets/images/elsevier.png"
            class="sponsor-logos__img"
            alt="Elsevier"
            class="sponsor-logo" />
        </a>

        <a href="https://www.ku.ac.ae/rich" target="_blank">
          <img
            src="assets/images/rich.png"
            class="sponsor-logos__img"
            alt="Rich"
            class="sponsor-logo" />
        </a>
        <a href="https://www.ku.ac.ae/cmat" target="_blank">
          <img
            src="assets/images/cmat.png"
            class="sponsor-logos__img"
            alt="Cmat"
            class="sponsor-logo" />
        </a>
      </div>
    </div>
  </section>

  <!-- info section   -->
  <section class="info-section">
    <div class="container info-section__content">
      <div class="info-card animate-on-scroll" data-animate="fade-right">
        <img
          src="assets/images/abu-dhabi.png"
          alt="Abu Dhabi skyline"
          class="info-card__image" />

        <div class="info-card__content">
          <h1 class="info-card__title">About Abu Dhabi</h1>
          <div class="info-card__text-contents">
            <div>
              <p class="info-card__text">
                The name Abu Dhabi means ‘Land of the Gazelle’ in Arabic. The
                story goes that the emirate was founded when a young antelope
                led a wandering tribe to fresh water on an island with no more
                than 300 Barasti (palm frond) huts, a few coral buildings and
                the Ruler’s fort.
              </p>
              <p class="info-card__text">
                That modest island settlement has since been transformed into
                the modern, cosmopolitan, high-rise UAE capital city.
              </p>
              <p class="info-card__text">
                The gorgeous archipelago offers a mix of ancient history, rich
                culture and heritage, stunning nature, beautiful beaches,
                high-end luxury, surprising wildlife, family-friendly
                adventure, world-class shopping, renowned golf courses and a
                welcoming atmosphere.
              </p>
            </div>

            <div>
              <a
                class="button-shape button-shape-primary"
                href="#registration">
                Register Now
              </a>
            </div>
          </div>
        </div>
      </div>

      <div class="info-card animate-on-scroll" data-animate="fade-left">
        <img
          src="assets/images/khalifa-university.png"
          alt="Khalifa University campus"
          class="info-card__image" />

        <div class="info-card__content">
          <h1 class="info-card__title">About KU</h1>
          <div class="info-card__text-contents">
            <div>
              <p class="info-card__text">
                Khalifa University of Science and Technology is an
                internationally top-ranked research intensive university
                located in Abu Dhabi, UAE. It boasts three colleges, three
                research institutes, 20 research centers, and 37 departments
                covering a broad range of disciplines in science, engineering,
                and medicine. Khalifa University is the top-ranked university
                in the UAE. Its research and academic offerings address the
                entire range of strategic, scientific and industrial
                challenges of the 21st century.
              </p>
              <p class="info-card__text">
                Khalifa University’s world-class faculty and state-of-the-art
                research facilities provide an unparalleled learning
                experience to students around the world. The university brings
                together the best in science, engineering and medicine in the
                UAE, to offer specialized degrees that can take promising high
                school graduates all the way to top-ranked.
              </p>
            </div>
            <div>
              <a
                class="button-shape button-shape-primary"
                href="#registration">
                Register Now
              </a>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

  <!-- travel-section -->
  <section class="travel-section animate-on-scroll" data-animate="fade-left">
    <div class="container travel-section__contents">
      <div class="travel-section__content">
        <h1 class="travel-section__title">Visa & Travel</h1>
        <p class="travel-section__text">
          Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
          eiusmod tempor incididunt ut labore et dolore magna aliqua. Quis
          ipsum suspendisse ultrices gravida. Risus commodo viverra maecenas
          accumsan lacus vel facilisis.
        </p>
      </div>

      <div class="travel-section__image-wrapper">
        <div class="travel-section__image-wrapper__bg-overlay"></div>
        <img
          src="assets/images/travel.png"
          alt="Traveler pulling luggage"
          class="travel-section__image" />
      </div>
    </div>
  </section>

  <!-- venue-section -->
  <section
    id="venue"
    class="venue-section animate-on-scroll"
    data-animate="fade-right">
    <div class="container venue-section__contents">
      <div class="venue-section__content">
        <div class="venue-section__content__info">
          <h1 class="venue-section__title">Venue Details</h1>
          <p class="venue-section__text">Khalifa University, Main Campus</p>
        </div>
      </div>

      <div class="venue-section__map-wrapper">
        <iframe
          src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3537.5129408825!2d54.39233387524514!3d24.447355578201226!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3e5e686120c31b23%3A0x5d25df2e9ffa142e!2sKhalifa%20University!5e1!3m2!1sen!2sae!4v1758454530160!5m2!1sen!2sae"
          width="100%"
          height="300"
          style="border: 0"
          allowfullscreen
          loading="lazy"
          referrerpolicy="no-referrer-when-downgrade"></iframe>
      </div>
    </div>
  </section>

  <!-- accommodation-section -->
  <section class="accommodation-section">
    <div class="container accommodation-section__contents">
      <div class="accommodation-section__header">
        <h1
          class="accommodation-section__title animate-on-scroll"
          data-animate="fade-right">
          Recommended <br />
          Accommodation
        </h1>
        <p
          class="accommodation-section__subtitle animate-on-scroll"
          data-animate="fade-left">
          The following accommodations are a walkable distance from the
          conference venue
        </p>
      </div>

      <div class="accommodation-section__grid">
        <div class="animate-on-scroll" data-animate="fade-up">
          <a
            href=" https://maps.app.goo.gl/CN4Y4sEcciy6AxC8A"
            target="_blank"
            class="accommodation-card">
            <img
              src="assets/images/dusit-thani.png"
              alt="Dusit Thani Abu Dhabi"
              class="accommodation-card__image" />
            <div class="accommodation-card__detail">
              <h3 class="accommodation-card__name">Dusit Thani Abu Dhabi</h3>

              <a
                href=" https://maps.app.goo.gl/CN4Y4sEcciy6AxC8A"
                target="_blank">
                <button
                  class="button-shape button-shape-primary accommodation-card__button">
                  view map
                </button>
              </a>
            </div>
          </a>
        </div>

        <div class="animate-on-scroll" data-animate="fade-up">
          <a
            href="https://maps.app.goo.gl/rKsbjYrTbXdVRKGY6"
            target="_blank"
            class="accommodation-card">
            <img
              src="assets/images/al-jazeera-hotel.png"
              alt="Al Jazeera Club Hotel Abu Dhabi"
              class="accommodation-card__image" />

            <div class="accommodation-card__detail">
              <h3 class="accommodation-card__name">
                Al Jazeera Club Hotel Abu Dhabi
              </h3>
              <a
                href="https://maps.app.goo.gl/rKsbjYrTbXdVRKGY6"
                target="_blank">
                <button
                  class="button-shape button-shape-primary accommodation-card__button">
                  view map
                </button>
              </a>
            </div>
          </a>
        </div>
        <div class="animate-on-scroll" data-animate="fade-up">
          <a
            href="https://www.google.com/maps/place/Al+Nakheel+Hotel+Apartments+Abu+Dhabi/@24.4770776,54.3663919,17z/data=!4m10!3m9!1s0x3e5e663faa62719d:0xf47f623f36653c19!5m3!1s2025-10-05!4m1!1i2!8m2!3d24.4770728!4d54.3712628!16s%2Fg%2F1tfj1nfm?entry=ttu&g_ep=EgoyMDI1MDkyOS4wIKXMDSoASAFQAw%3D%3D"
            target="_blank"
            class="accommodation-card">
            <img
              src="assets/images/al-nakheel-hotel.jpg"
              alt="Al Jazeera Club Hotel Abu Dhabi"
              class="accommodation-card__image" />

            <div class="accommodation-card__detail">
              <h3 class="accommodation-card__name">Al Nakheel Hotel</h3>
              <a
                href="https://www.google.com/maps/place/Al+Nakheel+Hotel+Apartments+Abu+Dhabi/@24.4770776,54.3663919,17z/data=!4m10!3m9!1s0x3e5e663faa62719d:0xf47f623f36653c19!5m3!1s2025-10-05!4m1!1i2!8m2!3d24.4770728!4d54.3712628!16s%2Fg%2F1tfj1nfm?entry=ttu&g_ep=EgoyMDI1MDkyOS4wIKXMDSoASAFQAw%3D%3D"
                target="_blank">
                <button
                  class="button-shape button-shape-primary accommodation-card__button">
                  view map
                </button>
              </a>
            </div>
          </a>
        </div>
      </div>
    </div>
  </section>

  <!-- footer section  -->
  <?php include 'includes/footer.php'; ?>

  <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.7.1/jquery.min.js"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.3.4/owl.carousel.min.js"></script>

  <script src="assets/js/main.js"></script>
  <script src="assets/js/slider.js"></script>
</body>

</html>